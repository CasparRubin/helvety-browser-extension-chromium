...
**“100% client-side” for user content** means: this extension **never** sends decrypted titles, notes, contact details, or URLs to Helvety app APIs or PostgREST. Encryption and decryption run in the extension via Web Crypto through `@helvety/shared/crypto/e2ee-entity-crypto` (re-exported by `encrypt-entities.ts` / `decrypt-entities.ts` for stable import paths). That matches helvety.com.
...
`decrypt-entities.ts` re-exports `@helvety/shared/crypto/e2ee-entity-crypto` with **field-bound AAD** (`table:recordId:column` per `encrypted_*` column). Plaintext exists in extension memory and React state while unlocked.

**Vault lock vs sign-out (aligned with helvety.com):** idle vault lock calls `deleteMasterKey` + `clearAllKeys` in `use-extension-vault.ts` and wipes decrypted list/form state via `clearDecryptedEntityState` in `use-extension-entities.ts` (and `clearEntityFormState` in `use-extension-entity-form.ts`; `App.tsx` wires vault `onLocked` → wipe). PRF salt cache is **kept** for faster re-unlock. Sign-out calls `clearAllKeys` + `clearCachedPRFSalt`, clears weekly proof, and wipes decrypted state in `use-extension-auth.ts` / `use-extension-entities.ts`. The same in-memory wipe runs when `user_id` changes so another account never sees the previous user’s rows before the next fetch.
