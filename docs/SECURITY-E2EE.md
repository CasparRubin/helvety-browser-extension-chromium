...
Automated guards: **`entity-repository.test.ts`** (no star selects; ciphertext-only writes via `@helvety/shared/e2ee-write-guard`), **`tests/e2ee-crypto-wiring.test.ts`** (extension crypto re-exports `@helvety/shared/crypto/e2ee-entity-crypto`; roundtrips live in **`packages/shared`**), **`tests/e2ee-catalog-wiring.test.ts`**, **`tests/e2ee-forms-wiring.test.ts`**, **`tests/e2ee-types-wiring.test.ts`**, **`extension-entity-links-hooks.test.tsx`** (entity-link load/link/unlink failures toast via `getE2eeHookErrorMessage`), **`unlock-dev-log.test.ts`**, **`passkey-unlock.test.ts`** (no PRF in verify body; shared `backfillKeyCheckValueIfMissing`), **`src/lib/auth-session-policy-wiring.test.ts`**, **`src/lib/dead-export-cleanup.test.ts`**, **`scripts/check-extension-e2ee-consistency.mjs`**, **`scripts/check-extension-auth-consistency.mjs`**, **`tests/guardrail-scripts`**, **`tests/automation-policy-consistency.test.ts`**, **`tests/supabase-auth-patterns.test.ts`** (`getUser()`-first session), **`tests/copy-accuracy.test.ts`**, **`tests/readme-vendor-docs.test.ts`** (README must not claim read-only MVP or a separate detail-view step), **`tests/security-e2ee-docs.test.ts`** (this doc stays aligned with manifest and side panel), **`entity-rich-text-editor.test.ts`** (TipTap remount key + mount-only `content` parity with web `E2eeRichTextItemEditorShell`; co-located with the side panel UI), plus other UI/data tests under **`src/**/\*.test.ts(x)`** (navigation including `entityFormSessionKey`, hooks). Shared package tests cover **`e2ee-entity-columns`**, **`e2ee-write-guard`**, **`entity-links-client`**, **`e2ee-entity-crypto`**, **`entity-list-grouping`**, and **`link-tree-ops`**.
...
| **Session**             | Supabase JWT / refresh, HMAC weekly proof                                                        | Full session JSON + `helvety_extension_weekly_proof` in `chrome.storage.local`                | Not shown in About UI                          |

**“100% client-side” for user content** means: this extension **never** sends decrypted titles, notes, contact details, or URLs to Helvety app APIs or PostgREST. Encryption and decryption run in the extension via Web Crypto through `@helvety/shared/crypto/e2ee-entity-crypto` (re-exported by `encrypt-entities.ts`; `decrypt-entities.ts` re-exports that barrel for stable import paths). That matches helvety.com.
...
User **access tokens** after OTP sign-in live in the Supabase session JSON in `chrome.storage.local` (together with the refresh token). Sensitive at runtime; not entity plaintext.
...
`decrypt-entities.ts` re-exports the `encrypt-entities` barrel (shared `@helvety/shared/crypto/e2ee-entity-crypto` with **field-bound AAD** `table:recordId:column` per `encrypted_*` column). Plaintext exists in extension memory and React state while unlocked.

**Vault lock vs sign-out (aligned with helvety.com):** idle vault lock calls `deleteMasterKey` + `clearAllKeys` in `use-extension-vault.ts` and wipes decrypted list/form state via `clearDecryptedEntityState` in `use-extension-entities.ts` (and `clearEntityFormState` in `use-extension-entity-form.ts`; `App.tsx` wires vault `onVaultUiReset` → wipe). PRF salt cache is **kept** for faster re-unlock. Sign-out calls `clearAllKeys` + `clearCachedPRFSalt`, clears weekly proof, and wipes decrypted state in `use-extension-auth.ts` / `use-extension-entities.ts`. The same in-memory wipe runs when `user_id` changes so another account never sees the previous user’s rows before the next fetch.
...
Create, update, and delete entity **content** use **direct Supabase PostgREST** (`entity-repository.ts`) with payloads from `encrypt-entities.ts` (shared `e2ee-entity-crypto` re-export). Only **serialized ciphertext** in `encrypted_*` columns; `assertEncryptedWritePayloadAuto` from `@helvety/shared/e2ee-write-guard` runs before every insert/update. Structural metadata and default ids come from `@helvety/shared/e2ee-entity-defaults`. Empty create forms use `@helvety/shared/e2ee-create-inputs` (same as web zones). **Create** passes an optional `clientRecordId` so ciphertext AAD matches the row id; the UI is **save-first** (empty form until Save, then edit mode). Link URLs are normalized client-side before encryption (`link-url-normalize.ts` re-exports `@helvety/shared/e2ee-url-normalize`).
...
| `host_permissions` | Project Supabase origin (`bkdzeihxzvrkndjvyzye.supabase.co`), `helvety.com`, and `*.helvety.com` (auth + gateway / subdomain deep links)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
