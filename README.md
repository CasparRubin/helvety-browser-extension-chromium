...
- Metadata pickers: `catalog-picker.tsx` — colored stage/label/category/priority chip buttons (`@helvety/ui/Button`) aligned with web apps.
...
pnpm type-check
pnpm ci:check          # format:check → lint → type-check → test → consistency:extension-auth → consistency:extension-e2ee
...
| Path                         | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/lib/`                   | Helvety auth API (`helvety-auth-api.ts`), Supabase session (`extension-supabase.ts`), passkey unlock, E2EE crypto/URL/tree re-exports (`encrypt-entities.ts`, `decrypt-entities.ts`, `link-url-normalize.ts`, `link-tree.ts`), delete-message registry (`entity-config.ts`), entity + link repositories, config, session policy (`auth-session-policy-wiring.test.ts`), dead-export cleanup (`dead-export-cleanup.test.ts`)                                                                                                     |
